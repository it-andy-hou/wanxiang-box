...
(function() {
    ...
    // 绑定事件
    function bindEvents() {
        console.log('绑定命令执行事件...');
        
        // 创建任务按钮
        const createTaskBtn = document.getElementById('createCommandTaskBtn');
        if (createTaskBtn) {
            createTaskBtn.addEventListener('click', showTaskConfig);
        }

        // 定时执行复选框事件
        const scheduleEnabled = document.getElementById('commandScheduleEnabled');
        if (scheduleEnabled) {
        ...
        // 开始执行按钮
        const startExecutionBtn = document.getElementById('startCommandExecutionBtn');
        if (startExecutionBtn) {
            startExecutionBtn.addEventListener('click', startExecution);
        }

        // 停止执行按钮
        const stopExecutionBtn = document.getElementById('stopCommandExecutionBtn');
        if (stopExecutionBtn) {
            stopExecutionBtn.addEventListener('click', stopExecution);
        }

        // 取消任务按钮
        const cancelTaskBtn = document.getElementById('cancelCommandTaskBtn');
        if (cancelTaskBtn) {
            cancelTaskBtn.addEventListener('click', cancelTask);
        }

        // 清空日志按钮
        const clearLogBtn = document.getElementById('clearCommandLogBtn');
        if (clearLogBtn) {
            clearLogBtn.addEventListener('click', clearLog);
        }

        // 下载日志按钮
        const downloadLogBtn = document.getElementById('downloadCommandLogBtn');
        if (downloadLogBtn) {
            downloadLogBtn.addEventListener('click', downloadLog);
        }

        // 复制日志按钮
        const copyLogBtn = document.getElementById('copyCommandLogBtn');
        if (copyLogBtn) {
            copyLogBtn.addEventListener('click', copyLog);
        }

        // 导出结果按钮
        const exportResultBtn = document.getElementById('exportCommandResultBtn');
        if (exportResultBtn) {
    ...
    function showTaskConfig() {
        const configSection = document.getElementById('commandTaskConfigSection');
        const historySection = document.getElementById('commandTaskHistorySection');
    ...
    function resetTaskForm() {
        ...
        const taskName = document.getElementById('commandTaskName');
        const taskConcurrency = document.getElementById('commandTaskConcurrency');
        const taskTimeout = document.getElementById('commandTaskTimeout');
        
        if (commandContent) commandContent.value = '';
        if (taskName) taskName.value = '';
        // 重置为全局设置的默认值
        resetExecutionParamsToGlobal(taskConcurrency, taskTimeout);
        
        // 隐藏监控区域
        const monitorSection = document.getElementById('commandTaskMonitorSection');
        if (monitorSection) {
    ...
    function handleScheduleToggle(e) {
        const scheduleTimeSection = document.getElementById('commandScheduleTimeSection');
        if (e.target.checked) {
            scheduleTimeSection.style.display = 'block';
            // 设置默认时间为当前时间+1小时
            const scheduleTimeInput = document.getElementById('commandScheduleTime');
    ...
    async function startExecution() {
        try {
            // 验证表单
            const commandContent = document.getElementById('commandContent');
            const commandText = commandContent ? commandContent.value.trim() : '';
            
            if (!commandText) {
                showMessage('请填写命令内容', 'error');
                return;
            }
            ...
            const taskName = `临时命令_${dateStr}`;

            // 设置任务名称
            const taskNameInput = document.getElementById('commandTaskName');
            if (taskNameInput) {
                taskNameInput.value = taskName;
            }

            // 获取配置
            const concurrencyInput = document.getElementById('commandTaskConcurrency');
            const timeoutInput = document.getElementById('commandTaskTimeout');
            ...
            // 检查是否使用sudo执行
            const useSudoCheckbox = document.getElementById('commandUseSudo');
            ...
            const taskData = {
                task_name: taskName,
                script_id: null,  // 临时命令没有script_id
                script_content: commandText,  // 临时命令内容
                is_temporary_command: true,  // 标记为临时命令
            ...
            console.log('准备创建临时命令任务，任务数据:', JSON.stringify(taskData, null, 2));
            ...
            // 切换按钮状态
            const startBtn = document.getElementById('startCommandExecutionBtn');
            const stopBtn = document.getElementById('stopCommandExecutionBtn');
            ...
            // 开始执行
            addLog(`[${formatTime(new Date())}] 开始执行临时命令任务: ${taskName}`, 'info');
    ...
    function initMonitorUI() {
        ...
        currentTaskResults = [];
        // 清空结果视图
        clearResultView('command');
    }

    // 更新进度
    function updateProgress(completed, total) {
        const progressFill = document.getElementById('commandProgressFill');
        const overallProgress = document.getElementById('commandOverallProgress');
        const totalCount = document.getElementById('commandTotalCount');
    ...
    function handleProgressUpdate(data) {
        ...
        if (status === 'success' || status === 'failed') {
            ...
            // 实时更新结果视图
            addResultCard('command', { host_ip: hostIp, status, stdout: stdout || '', stderr: stderr || '', duration });
        } else if (status === 'running') {
            addResultCard('command', { host_ip: hostIp, status: 'running', stdout: '', stderr: '', duration: 0 });
        }
        ...
        // 更新成功/失败计数
        const successCount = document.getElementById('commandSuccessCount');
        const failedCount = document.getElementById('commandFailedCount');
    ...
    function handleTaskComplete(result) {
        ...
        // 自动对比多台机器的执行结果
        showResultComparisonBanner('command', currentTaskResults);
    ...
    function startExecutionTimer() {
        ...
        executionTimer = setInterval(() => {
            ...
            const timeDisplay = document.getElementById('commandExecutionTime');
            if (timeDisplay) {
    ...
    function addLog(message, type = 'info') {
        const logDiv = document.getElementById('commandExecutionLog');
        if (!logDiv) return;
    ...
    function clearLog() {
        ...
        // 重置筛选/搜索状态
        window._commandCurrentFilter = 'all';
        window._commandSearchKeyword = '';
        const filterGroup = document.getElementById('commandFilterGroup');
        if (filterGroup) {
            filterGroup.querySelectorAll('.filter-btn').forEach(b => b.classList.remove('active'));
            const allBtn = filterGroup.querySelector('[data-filter="all"]');
            if (allBtn) allBtn.classList.add('active');
        }
        const searchInput = document.getElementById('commandResultSearch');
    ...
    async function downloadLog() {
        ...
        try {
            ...
            const taskNameInput = document.getElementById('commandTaskName');
            let taskName = taskNameInput ? taskNameInput.value : '临时命令日志';
    ...
    async function exportResults() {
        ...
        try {
            ...
            currentTaskResults.forEach(result => {
                ...
                // Excel单元格文本限制为32767字符，超出则截断并提示
                const MAX_CELL_LENGTH = 32767;
                if (output.length > MAX_CELL_LENGTH) {
                    const truncateMsg = `\n\n[输出过长已截断，共 ${output.length} 字符，仅显示前 ${MAX_CELL_LENGTH - 100} 字符]`;
                    output = output.substring(0, MAX_CELL_LENGTH - truncateMsg.length) + truncateMsg;
                }
            ...
            // 生成文件名
            const taskNameInput = document.getElementById('commandTaskName');
            let taskName = taskNameInput ? taskNameInput.value : '命令结果';
            taskName = taskName.replace(/[<>:"\/\\|?*]/g, '_');
    ...
    async function loadTaskHistory() {
        try {
            ...
            // 只显示临时命令任务
            const commandTasks = tasks.filter(task => task.is_temporary_command);
            renderTaskHistory(commandTasks);
        } catch (error) {
    ...
    function renderTaskHistory(tasks) {
        const tbody = document.querySelector('#commandTaskHistoryTable tbody');
        if (!tbody) return;
        ...
        tbody.innerHTML = sortedTasks.map(task => {
            // 对于临时命令，显示命令内容而不是任务名称
            let displayName = escapeHtml(task.task_name);
            let titleAttr = '';
            
            if (task.script_content) {
                const commandContent = task.script_content.trim();
                const maxLength = 50;
                
                if (commandContent.length <= maxLength) {
                    displayName = escapeHtml(commandContent);
                } else {
                    displayName = escapeHtml(commandContent.substring(0, maxLength)) + '...';
                    titleAttr = ` title="${escapeHtml(commandContent)}"`;
                }
            }
            ...
            <tr class="task-row-clickable" data-task-id="${task.id}" style="cursor: pointer;">
                <td>${task.id}</td>
                <td${titleAttr}><strong>${displayName}</strong></td>
                ...
                <td>
                    <button class="btn btn-sm btn-warning btn-reexecute-cmd" data-task-id="${task.id}" title="根据此历史命令预填配置，重新执行一遍" style="white-space:nowrap;">再执行</button>
                </td>
            </tr>
            `;
        }).join('');
    ...
    function bindTaskHistoryEvents() {
        document.querySelectorAll('.task-row-clickable').forEach(row => {
            row.addEventListener('click', async (e) => {
                ...
                if (e.target.closest('.btn-reexecute-cmd')) return;
        ...
        // 再执行按钮
        document.querySelectorAll('.btn-reexecute-cmd').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                e.stopPropagation();
                const taskId = parseInt(e.currentTarget.dataset.taskId);
                await reExecuteCommand(taskId);
            });
        });
    }
    ...
    // 根据历史命令预填配置，再执行一遍
    async function reExecuteCommand(taskId) {
        try {
            // 1. 获取历史任务详情
            const task = await taskService.getTask(taskId);
            if (!task) {
            ...
            // 2. 校验命令内容是否存在
            if (!task.script_content || !task.script_content.trim()) {
                showMessage('该历史任务没有命令内容，无法再执行', 'error');
                return;
            }
            ...
            // 5. 预填命令内容
            const commandInput = document.getElementById('commandContent');
            if (commandInput) {
                commandInput.value = task.script_content;
            }
            ...
            // 命令内容截断用于提示
            const cmdPreview = task.script_content.trim().replace(/\s+/g, ' ');
            const cmdDisplay = cmdPreview.length > 40 ? cmdPreview.slice(0, 40) + '…' : cmdPreview;
            showMessage(`已从历史命令「${cmdDisplay}」预填配置，请确认后点击"开始执行"`, 'info');
        } catch (error) {
            ...
            <div class="modal" style="max-width: 1000px;">
                ...
                <div class="modal-body">
                    ...
                    <div style="margin-bottom: 1rem;">
                        ...
                        <pre id="commandContent" style="background: var(--surface-subtle); padding: 0.5rem; border-radius: 6px; max-height: 150px; overflow: auto;">${escapeHtml(task.script_content)}</pre>
                    </div>
                    ` : ''}
    ...
    function initResultViewEvents() {
        // 视图切换
        const logBtn = document.getElementById('commandViewLogBtn');
        const resultBtn = document.getElementById('commandViewResultBtn');
        if (logBtn) logBtn.addEventListener('click', () => switchView('command', 'log'));
        if (resultBtn) resultBtn.addEventListener('click', () => switchView('command', 'result'));
        ...
        if (filterGroup) {
            filterGroup.querySelectorAll('.filter-btn').forEach(btn => {
                btn.addEventListener('click', () => {
                    ...
                    window._commandCurrentFilter = btn.dataset.filter;
                    applyResultFilter('command');
                });
            });
        }
        ...
        if (searchInput) {
            ...
            searchInput.addEventListener('input', () => {
                ...
                searchTimer = setTimeout(() => {
                    window._commandSearchKeyword = searchInput.value.trim().toLowerCase();
                    applyResultFilter('command');
                }, 300);
            });
        }
    ...
    function updateResultCard(cardEl, result) {
        ...
        const prefix = cardEl.closest('#commandResultContainer') ? 'command' : 'script';
    ...
    function applyResultFilter(prefix) {
        ...
        const filter = window._commandCurrentFilter || 'all';
        const keyword = window._commandSearchKeyword || '';
    ...
    window.CommandExecution = {
        init: function() {
            if (!this._initialized) {
                ...
                console.log('CommandExecution 初始化');
                init();
            } else {
                console.log('CommandExecution 重新加载数据');
                loadTaskHistory();
            }
        },
        _initialized: false
